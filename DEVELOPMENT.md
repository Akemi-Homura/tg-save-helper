...
- `/syncsaved`、`/streamsaved` 及两个 watch 入口必须按旧到新边扫描边处理，禁止先把全部历史消息读入内存。
- 每处理成功一条消息或一个完整相册，立即写数据库并推进 pending-command 断点；重启从首条未完成消息恢复。
- 持久监听记录在 `saved_watches`，备份和转换现场分别记录在 `saved_backup`、`saved_stream`。
- watch 先完成 `count|all|from` 补扫，再持续监听新收藏消息。
- 程序生成的流媒体副本不得再次触发 `/watchstreamsaved`。
- 视频小于 5 GB 且预计输入、输出临时文件不会让文件系统达到 90% 时才允许下载。
- 进度消息只在开始、结束、异常和每扫描 500 条时发送。
- Telegram 消息链接必须以数字消息 ID 结尾，标点另起文本，避免手机客户端把右括号包含进 URL。
